...
public class PvpClassWarmupTimer extends PlayerTimer implements Listener{
	...
	public PvpClassWarmupTimer(HCF plugin){
		super(ConfigurationService.PVP_CLASS_WARMUP_TIMER, TimeUnit.SECONDS.toMillis(01), false);
		this.plugin = plugin;
		this.classWarmups = CacheBuilder.newBuilder().expireAfterWrite(this.defaultCooldown + 0001, TimeUnit.MILLISECONDS).build().asMap();
